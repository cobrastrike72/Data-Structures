...
class Heap{ // ascending order // the root is the minimum value
private:    
    vector <int> tree;
public:
    Heap(){}
    Heap(const vector<int> & vec) : tree(vec){
        ...
        for(int i = (j - 1)/ 2; i >= 0; i--) sift_down(i); // to get the minimum on the top
    }
    ...
    void sift_down(int parent_indx){ // used in case we are poping
        ...
        if(right_child_indx < siz && tree[left_child_indx] > tree[right_child_indx])
            swap(tree[left_child_indx], tree[right_child_indx]); // that means that i'm sure that the left child node is always smaller than the right child node
